...
In this model, the failure of one component will induce the failure rate increment of other components coupled to it. The impact of neighboring failures on the failure rate of each component is characterized by a sublinear (0<η<1), linear (η=1) , or superlinear (η>1) relationship. When η=1, our model corresponds to the Marshall-Olkin model only considering pairwise interaction, which is widely used in modeling failure coupling system. 
...
For example, “lineBA_shape=5.py” is xxxxcode for simulating BA network with linear interaction (η=1). 
“supBA_shape=5.py” is code for simulating BA network with superlinear interaction (η=2). 
“subBA_shape=5.py” is code for simulating BA network with sublinear interaction (η=0.5). 
